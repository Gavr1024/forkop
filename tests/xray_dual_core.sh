...
require_contains "$FORKOP_LIB/xray/runtime.uc" 'uci_xray_section_count() == 0' \
  "xray init-config must skip generation when no Xray sections are enabled"
require_contains "$FORKOP_LIB/xray/runtime.uc" 'logger' \
  "xray runtime errors must go to syslog via logger"
require_contains "$LIFECYCLE_UC" 'Preparing Xray sidecar' \
...
require_contains "$FORKOP_LIB/xray/outbound.uc" 'echConfigList' \
  "Xray TLS must keep the share-link ech= parameter"
...
require_contains "$LIFECYCLE_UC" 'module_success(XRAY_UC, [ "start-runtime" ])' \
  "lifecycle must start xray before sing-box"
