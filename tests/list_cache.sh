...
require_text "$CONFIG" "persist_lists_locally"
require_text "$SETTINGS_JS" "persist_lists_locally"
require_text "$SETTINGS_JS" "Save selected lists locally"
...
require_text "$CACHE_UC" "curl_fetch"
...
for po in "$PO_SRC" "$PO_PKG"; do
  ...
  require_text "$po" "Local list cache"
  require_text "$po" "Локальный кэш списков"
done
